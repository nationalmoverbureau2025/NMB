...
export async function generateReportPDF(reportData: ICompanyReport) {
  ...
  function addFooter() {
    ...
    for (let i = 1; i <= pageCount; i++) {
      doc.setPage(i);
      doc.setFontSize(10);
      doc.text('This report is generated by National Mover Bureau AI-powered verification system.', 20, pageHeight - footerHeight);
      doc.text('For more information visit: www.nationalmoverbureau.org', 20, pageHeight - footerHeight + 5);
    }
  }
  ...
  if (reportData.dba_names.length > 0) {
    for (const dba of reportData.dba_names) {
      checkPageOverflow(6);
      doc.text(`- ${dba.name} (From: ${dba.start_date} To: ${dba.end_date})`, 22, y);
      y += 6;
    }
  } else {
    checkPageOverflow(6);
  ...
  if (reportData.authority_statuses.length > 0) {
    for (const status of reportData.authority_statuses) {
      checkPageOverflow(6);
      doc.text(`- ${status.type}: ${status.status} (Updated: ${status.updated_at})`, 22, y);
      y += 6;
    }
  } else {
    checkPageOverflow(6);
  ...
  if (reportData.insurance.length > 0) {
    for (const ins of reportData.insurance) {
      checkPageOverflow(6);
      doc.text(`- ${ins.type}: ${ins.status}, Exp: ${ins.expiration_date}`, 22, y);
      y += 6;
    }
  } else {
    checkPageOverflow(6);
  ...
  const aiFlags = [
    ...
    { label: 'Reviewer Credibility', arr: reportData.reviewer_credibility_flags },
    ...
    { label: 'Sentiment Specificity', arr: reportData.sentiment_specificity_flags },
  ];
  for (const flag of aiFlags) {
    checkPageOverflow(6);
    doc.text(`${flag.label}: ${flag.arr.length > 0 ? flag.arr.join(', ') : 'None'}`, 22, y);
    y += 6;
  }
  ...
  const redFlags = [
    ...
    { label: 'Recent/Private Domain', value: reportData.recent_or_private_domain },
  ...
  // Save the PDF
  doc.save(`${reportData.companies_perfsol.company_name}-${reportData.id.split('-')[0]}-report.pdf`);
}