...
export async function getCompanyAnalytics(companyId: string) {
  ...
  try {
    ...
    const complaintTrends = complaints?.reduce((acc: any, curr) => {
      ...
      acc[month].total++;
      acc[month].byType[curr.complaint_type] = (acc[month].byType[curr.complaint_type] || 0) + 1;
      return acc;
    }, {});
    ...
    const reviewTrends = reviews?.reduce((acc: any, curr) => {
      ...
      acc[month].total++;
      acc[month].avgRating = (acc[month].avgRating * (acc[month].total - 1) + curr.rating) / acc[month].total;
...
export async function getRiskMetrics(companyId: string) {
  try {
    ...
    const reviewReliability = reviews.length ? (1 - suspiciousReviews / reviews.length) * 100 : 100;
